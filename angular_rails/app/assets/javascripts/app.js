...
app.factory("playerFactory", function($http){
   ...
   // similar to the create and index methods, we are passing the information that we get back from the rails PlayersController to the angular playersController
...
app.controller("playersController", function($scope, playerFactory){
    ...
    // calling the create method from our factory when the button is cliked and updating the $scope.players information with the json data that we get back from the rails PlayersController
...
app.factory("userFactory", function($http){
    ...
    factory.create = function(callback){
        $http.post("users").success(function(output){
            callback(output);
        })
    }
    return factory;
...
app.controller("UsersController", function($scope, userFactory){
    $scope.createUser = function($scope.newUser, function(json){
        $scope.users = json;
        $scope.newUser = {}
    })
})
